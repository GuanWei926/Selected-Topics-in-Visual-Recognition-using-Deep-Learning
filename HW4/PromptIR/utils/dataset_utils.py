...
class PromptTrainDataset(Dataset):
    ...
    def _init_ids(self):
        if (
            'denoise_15' in self.de_type or
            'denoise_25' in self.de_type or
            'denoise_50' in self.de_type
        ):
    ...
    def _init_clean_ids(self):
        ...
        clean_ids += [
            os.path.join(self.args.denoise_dir, id_)
    ...
    def _init_snow_ids(self):
        temp_ids = []
        snow_txt = self.args.data_file_dir + "snowy/snowTrain.txt"
        temp_ids += [
            self.args.desnow_dir + id_.strip()
            for id_ in open(snow_txt)
        ]
        self.snow_ids = [
            {"clean_id": x, "de_type": 6}
            for x in temp_ids
        ]
        self.snow_ids = self.snow_ids * 1  # 根據情況調整複製倍率

        self.snow_counter = 0
        self.num_snow = len(self.snow_ids)
        print("Total Snowy Ids : {}".format(self.num_snow))
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            clean_img = crop_img(
                np.array(Image.open(clean_id).convert('RGB')),
                base=16
            )
            ...
            if de_id == 3:
                # Rain Streak Removal
                degrad_img = crop_img(
                    np.array(Image.open(sample["clean_id"]).convert('RGB')),
                    base=16
                )
                clean_name = self._get_gt_name(sample["clean_id"])
                clean_img = crop_img(
                    np.array(Image.open(clean_name).convert('RGB')),
                    base=16
                )
            elif de_id == 4:
                ...
                clean_name = (
                    sample["clean_id"]
                    .replace("degraded", "clean")
                    .replace("snow-", "snow_clean-")
                )
...
class PromptValDataset(Dataset):
    def __init__(self, args):
        ...
        self.de_type_map = {'derain': 3, 'desnow': 6}
        self.toTensor = ToTensor()

        # 根據 de_type 決定要載入哪些 validation 圖片
        if 'derain' in args.de_type:
            self._init_rs_ids()
        if 'desnow' in args.de_type:
            self._init_snow_ids()

    def _init_rs_ids(self):
        val_file = os.path.join(self.args.data_file_dir, "rainy/rainVal.txt")
        with open(val_file) as f:
            for line in f:
                ...
                clean_path = (
                    degraded_path.replace("degraded", "clean")
                    .replace("rain-", "rain_clean-")
                )
                self.sample_ids.append({
                    "degraded": degraded_path,
                    "clean": clean_path,
                    "de_type": self.de_type_map['derain']
                })

    def _init_snow_ids(self):
        val_file = os.path.join(self.args.data_file_dir, "snowy/snowVal.txt")
        with open(val_file) as f:
            for line in f:
                degraded_path = os.path.join(
                ...
                self.sample_ids.append({
                    "degraded": degraded_path,
                    "clean": clean_path,
                    "de_type": self.de_type_map['desnow']
                })

    def __getitem__(self, idx):
        sample = self.sample_ids[idx]
        degraded_img = crop_img(
            np.array(Image.open(sample["degraded"]).convert('RGB')),
            base=16
        )
        clean_img = crop_img(
            np.array(Image.open(sample["clean"]).convert('RGB')),
            base=16
        )
        ...
        name = sample["degraded"].split('/')[-1]
        return [name, sample["de_type"]], degraded_img, clean_img
...
class DenoiseTestDataset(Dataset):
    ...
    def __getitem__(self, clean_id):
        ...
        clean_img, noisy_img = (
            self.toTensor(clean_img),
            self.toTensor(noisy_img)
        )
...
class DerainDehazeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        clean_img, degraded_img = (
            self.toTensor(clean_img),
            self.toTensor(degraded_img)
        )
