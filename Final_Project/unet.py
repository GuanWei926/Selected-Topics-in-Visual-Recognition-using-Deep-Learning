...
def predict(model, img_paths: List[Path], out_path: Path,
    ...
    def predict(arg):
        ...
        def make_batch(xy_batch_):
            return (xy_batch_, torch.stack([
                utils.img_transform(
                    img[y: y + s, x: x + s]) for x, y in xy_batch_]))
    ...
    def save_prediction(arg):
        ...
        if not downsampled:
            pred_img = np.stack(
                [utils.downsample(p, PRED_SCALE) for p in pred_img])
...
def save_predictions(root: Path, n: int, inputs, targets, outputs):
    ...
    for i in range(batch_size):
        prefix = str(
            root.joinpath('{}-{}'.format(str(n).zfill(2), str(i).zfill(2))))
        utils.save_image(
            '{}-input.jpg'.format(prefix),
            skimage.exposure.rescale_intensity(
                inputs_data[i], out_range=(0, 1)))
        utils.save_image(
            '{}-output.jpg'.format(prefix), colored_prediction(
                outputs_probs[i]))
...
def main():
    ...
    arg('--mode', choices=[
        ...
        'predict_all_valid'], default='train')
    arg('--model-path',
    ...
    if args.mode == 'train':
        print('Training on {} images!'.format(len(train_paths)))
        ...
        if args.mode in {'predict_valid', 'predict_all_valid'}:
            if args.mode == 'predict_all_valid':
                # include all paths we did not train on
                # (makes sense only with --limit)
                new_paths = set(utils.labeled_paths()) - set(train_paths)
                valid_paths = list(set(valid_paths) | new_paths)

                print('Predicting {} valid images'.format(len(valid_paths)))
            ...
            out_path = root.joinpath('train')
            out_path.mkdir(exist_ok=True)
            predicted = {p.stem.split('-')[0] for p in out_path.glob('*.npy')}
            test_dir = utils.DATA_ROOT.joinpath('Test')
            test_paths = [
