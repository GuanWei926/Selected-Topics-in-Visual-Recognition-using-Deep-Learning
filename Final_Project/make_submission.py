...
def extract_blobs(cls_pred, thresholds, min_sigma=1, max_sigma=4, num_sigma=4):
    def process_threshold(threshold):
        blobs = blob_log(cls_pred, threshold=threshold,
                         min_sigma=min_sigma, max_sigma=max_sigma,
                         num_sigma=num_sigma)
        return [
            (x, y, cls_pred[int(round(y)), int(round(x))])
            for y, x, _ in blobs
        ]

    return [process_threshold(t) for t in thresholds]
...
def load_xs_ys(pred_path: Path, coords: pd.DataFrame,
    ...
    for cls in range(utils.N_CLASSES):
        ...
        try:
            ...
            cls_coords = []
        cls_blobs = extract_blobs(cls_pred, BLOB_THRESHOLDS)
        ...
        def steps(m):
            return range(-PATCH_SIZE + step, m + PATCH_SIZE - step, step)
...
def load_all_features(
    ...
    print('Loading features from', root)
    ...
    if only_valid:
        ...
        print("Example pred_paths:", pred_paths[:3])

        print(f"Valid IDs: {sorted(valid_ids)[:10]} ...")
        print(f"Pred IDs: {[get_id(p) for p in pred_paths]}")
    ...
    if not args.new_features and features_path.exists():
        ...
        print("ids:", ids)
        # print("data['ids']:", data['ids'])
        print("data['ids'][0]:", data['ids'][0])
    ...
    with multiprocessing.pool.Pool(processes=24) as pool:
        for id, scale, xs, ys, blobs, blob_ids in tqdm.tqdm(
                pool.imap(
                    partial(load_xs_ys, coords=coords),
    ...
    int_keys = {'ids', 'ys'}
    data = {
        k: np.array(v, dtype=np.int32 if k in int_keys else np.float32)
...
def train(data, *regs,
    ...
    for cls in range(utils.N_CLASSES):
        ...
        ys = data['ys'][cls]
        xs = input_features(
            concated_xs if concat_features else data['xs'][cls])

        try:
            pred, fitted = train_predict(regs, xs, ys, ids)
        except Exception as e:
            print(f"[ERROR] Class {cls} failed: {e}")
            continue
        print("lenth of pred:", len(pred))
        print("lenth of xs:", len(xs))
        print("lenth of ys:", len(ys))
        print("lenth of ids:", len(ids))
    ...
    print(
        f"{regs_name} with {', '.join(FEATURE_NAMES)} features: "
        f"mean patch RMSE {np.mean(all_patch_rmse):.3f}, "
        f"mean image RMSE {np.mean(all_rmse):.2f}, "
        f"mean baseline RMSE {np.mean(all_baselines):.2f}"
    )
    ...
    del fitted_regs, expl_by_cls, pred, fitted, pred_by_id
    del ys_by_id, xs, ys, ids
    gc.collect()
    ...
    for train_ids, valid_ids in cv.split(xs, groups=ids):
        ...
        for reg in regs:
            ...
            if isinstance(reg, XGBRegressor):
                reg = XGBRegressor(**reg.get_params())
                reg.set_params(
                    eval_metric='rmse',
                    n_estimators=500,
                    callbacks=[xgb_callback.EarlyStopping(rounds=3)],
                    verbosity=0,
                    n_jobs=2,
                )
                reg.fit(
                    xs[train_ids], ys[train_ids],
...
def predict(
    ...
    print("all_regs:", all_regs)
    items = list(enumerate(zip(classes, all_regs)))
    for cls, (cls_name, cls_regs) in tqdm.tqdm(items):
        print(f'Predicting for class {cls_name}...')
        ids = all_ids[cls]
        xs = input_features(concated_xs if concat_features else all_xs[cls])
        pred = average_predictions(
            [reg.predict(xs) for reg in tqdm.tqdm(cls_regs)])
        # print(f'Predicting for class {cls_name} with {len(cls_regs)} models')
        # print(pred)
        # print(get_pred_by_id(ids, pred, unique_ids))
        print("test!!!")
        print(round_prediction(get_pred_by_id(ids, pred, unique_ids)))
        all_preds[cls_name] = round_prediction(
            get_pred_by_id(ids, pred, unique_ids))
    print('Predictions shape:', all_preds.shape)
    print(all_preds['adult_males'])
    ...
    if args.mode == 'train':
        ...
        train(data,
              ExtraTreesRegressor(
                  ...
                  n_jobs=8, criterion='squared_error'),
              ExtraTreesRegressor(
