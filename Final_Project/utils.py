...
def load_image(path: Path, *, cache: bool) -> np.ndarray:
    ...
    if path.parent.name == 'Train':
        ...
        img_dotted = cv2.imread(
            str(path.parent.parent / 'TrainDotted' / path.name))
...
def train_valid_split(args, coords) -> Tuple[List[Path], List[Path]]:
    ...
    if args.stratified:
        ...
        img_paths.sort(
            key=lambda p: idx_by_id.get(int(p.stem), len(sorted_ids)))
        train, test = [], []
...
class BaseDataset(Dataset):
    def __init__(self,
        ...
        zipped = list(zip(self.img_ids, img_paths))
        self.imgs = {
            img_id: load_image(p, cache=True)
            for img_id, p in tqdm.tqdm(zipped, desc='Images')
        }

        valid_ids = [
            img_id for img_id in self.img_ids if img_id in coords.index]
        self.coords = coords.loc[valid_ids].dropna()
    ...
    def get_patch_points(self):
        ...
        if oversample:
            ...
            while item is None or item.name not in self.imgs:
                item = self.coords.iloc[
                    random.randint(0, len(self.coords) - 1)]
            img_id = item.name
        else:
        ...
        if oversample:
            ...
            try:
                x = random.randint(
                    max(x0 - s, b), min(x0 + s, max_x - (b + s)))
                y = random.randint(
                    max(y0 - s, b), min(y0 + s, max_y - (b + s)))
            except ValueError:
        ...
        assert patch.shape == (
            self.patch_size, self.patch_size, 3), patch.shape
        points = []
        if len(coords) > 0:
...
def train(args, model: nn.Module, criterion, *, train_loader, valid_loader,
    ...
    if make_optimizer is None:
        ...
        make_optimizer = default_optimizer
    ...
    for epoch in range(epoch, args.n_epochs + 1):
        ...
        try:
            ...
            if valid_loss < best_valid_loss:
                ...
                recent_min = min(valid_losses[-2:])
                if recent_min > best_valid_loss:
...
def save_image(fname, data):
    with warnings.catch_warnings():
        ...
        if data.dtype == np.float32 or data.dtype == np.float64:
            data = np.clip(data, 0, 1)  # if data is normalized between 0 and 1
...
def plot(*args, ymin=None, ymax=None, xmin=None, xmax=None, params=False,
    ...
    for path in paths:
        path = Path(path)
        log_path = path.joinpath('train.log')
        with json_lines.open(str(log_path), broken=True) as f:
            events = list(f)
        if params:
            print(path)
